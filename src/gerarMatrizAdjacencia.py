...
def gerar_matriz_adjacencia(arquivo_entrada, arquivo_saida):
    rodovia_ids = {}
    acidentes_por_rodovia = {}
    id_counter = 0
    ...
    try:
        with open(arquivo_entrada, mode='r', encoding='utf-8') as csvfile:
            ...
            for linha in linhas:
                ...
                acidentes_por_rodovia[rodovia] = total_acidentes
        ...
        for rodovia, conexoes in rodovias_conexoes.items():
            if rodovia in rodovia_ids:
                ...
                total_rodovia = acidentes_por_rodovia.get(rodovia, 0)

                for conectada in conexoes:
                    if conectada in rodovia_ids:
                        id_conectada = rodovia_ids[conectada]
                        total_conectada = acidentes_por_rodovia.get(conectada, 0)

                        peso = (total_rodovia + total_conectada) / 2
                        matriz[id_rodovia][id_conectada] = peso
                        print(f"Rodovia {rodovia} conectada com {conectada} com peso {peso}")
        ...
        print(f"Matriz de adjacência salva em {arquivo_saida}")
...
arquivo_saida = 'grafo_ligacoes_sem_coeficiente.csv'

gerar_matriz_adjacencia(arquivo_entrada, arquivo_saida)
