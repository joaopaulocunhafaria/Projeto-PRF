...
def gerar_dados_gephi(arquivo_entrada, arquivo_nos, arquivo_arestas):
    ...
    try:
        with open(arquivo_entrada, mode='r', encoding='utf-8') as csvfile:
            ...
            for linha in linhas:
                ...
                coef_fatalidade = acidentes_fatais / total_acidentes if total_acidentes > 0 else 0
                fatalidade_por_rodovia[rodovia] = coef_fatalidade

                if rodovia not in rodovia_ids:
                    rodovia_ids[rodovia] = id_counter
                    nos.append([id_counter, rodovia, "Rodovia", coef_fatalidade])
                    id_counter += 1

            for rodovia, conexoes in rodovias_conexoes.items():
                if rodovia in rodovia_ids:
                    id_rodovia = rodovia_ids[rodovia]
                    coef_fatalidade_rodovia = fatalidade_por_rodovia.get(rodovia, 0)

                    for rodovia_conectada in conexoes:
                        if rodovia_conectada in rodovia_ids:
                            id_conectada = rodovia_ids[rodovia_conectada]
                            coef_fatalidade_conectada = fatalidade_por_rodovia.get(rodovia_conectada, 0)

                            peso_aresta = (coef_fatalidade_rodovia + coef_fatalidade_conectada) / 2
                            arestas.append([id_rodovia, id_conectada, peso_aresta])
    ...
    with open(arquivo_nos, mode='w', encoding='utf-8', newline='') as csvfile:
        ...
        escritor.writerow(["Id", "Label", "Type", "CoefFatalidade"])
    ...
    print(f"Arquivos para Gephi gerados com sucesso: {arquivo_nos} e {arquivo_arestas}")

arquivo_entrada = 'rodoviasAcidentes.csv'
arquivo_nos = 'gephi_nos_rodovias_com_coeficiente.csv'
arquivo_arestas = 'gephi_arestas_rodovias_com_coeficiente.csv'
