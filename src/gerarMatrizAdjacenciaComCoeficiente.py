...
# Dicionário com as conexões das rodovias
...
rodovias_conexoes = {
    ...
    "RJ 116": ["SP 116", "RS 116", "RJ 101"],  # Corrigido: adicionado RJ 101
...
def gerar_matriz_adjacencia_fatalidade(arquivo_entrada, arquivo_saida):
    rodovia_ids = {}
    fatalidade_por_rodovia = {}
    id_counter = 0
    ...
    try:
        with open(arquivo_entrada, mode='r', encoding='utf-8') as csvfile:
            ...
            for linha in linhas:
                rodovia = linha[0]
                total_acidentes = int(linha[1])
                acidentes_fatais = int(linha[2])
                taxa_fatalidade = acidentes_fatais / total_acidentes if total_acidentes > 0 else 0
                fatalidade_por_rodovia[rodovia] = taxa_fatalidade

                if rodovia not in rodovia_ids:
                    rodovia_ids[rodovia] = id_counter
                    id_counter += 1

            for rodovia, conexoes in rodovias_conexoes.items():
                if rodovia not in rodovia_ids:
                    rodovia_ids[rodovia] = id_counter
                    id_counter += 1
                for conectada in conexoes:
                    if conectada not in rodovia_ids:
                        rodovia_ids[conectada] = id_counter
                        id_counter += 1
        ...
        for rodovia, conexoes in rodovias_conexoes.items():
            if rodovia in rodovia_ids:
                id_rodovia = rodovia_ids[rodovia]
                taxa_fatalidade_rodovia = fatalidade_por_rodovia.get(rodovia, 0)
                for conectada in conexoes:
                    if conectada in rodovia_ids:
                        id_conectada = rodovia_ids[conectada]
                        taxa_fatalidade_conectada = fatalidade_por_rodovia.get(conectada, 0)
                        peso = (taxa_fatalidade_rodovia + taxa_fatalidade_conectada) / 2
                        matriz[id_rodovia][id_conectada] = peso
                        matriz[id_conectada][id_rodovia] = peso
        ...
        with open(arquivo_saida, mode='w', encoding='utf-8', newline='') as csvfile:
            ...
            rodovias_ordenadas = sorted(rodovia_ids, key=rodovia_ids.get)
            escritor.writerow(["Rodovia"] + rodovias_ordenadas)

            for i, rodovia in enumerate(rodovias_ordenadas):
                escritor.writerow([rodovia] + list(matriz[i]))

        print(f"Matriz de adjacência baseada em fatalidade salva em {arquivo_saida}")
...
arquivo_entrada = 'src/rodoviasAcidentes.csv'
arquivo_saida = 'graphs/grafo_ligacoes_com_coeficiente.csv'

gerar_matriz_adjacencia_fatalidade(arquivo_entrada, arquivo_saida)
